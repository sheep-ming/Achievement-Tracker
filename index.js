...
// [핵심 변경] 저장소 키를 '번호'가 아닌 '고유 ID(파일명)'로 사용
function getCharData(uniqueId) {
    if (!uniqueId) {
        return {
    ...
    if (!extension_settings[MODULE_NAME].characterData[uniqueId]) {
        extension_settings[MODULE_NAME].characterData[uniqueId] = {
            enabled: true,
            toastEnabled: true,
            unlocked_assets: [], 
            cheated_assets: [],  
    ...
    if (!extension_settings[MODULE_NAME].characterData[uniqueId].cheated_assets) {
        extension_settings[MODULE_NAME].characterData[uniqueId].cheated_assets = [];
    }
    return extension_settings[MODULE_NAME].characterData[uniqueId];
}
...
async function scanChatHistory() {
    ...
    // [변경] this_chid 대신 char.avatar(파일명) 사용
    const uniqueId = char.avatar;
    const data = getCharData(uniqueId);
...
function handleCustomMsgChange() {
    const char = getCurrentCharacter();
    if (!char) return;
    const data = getCharData(char.avatar);
...
function handleReset() {
    ...
    if (!confirm(`[${char.name}]의 수집 기록을 초기화하시겠습니까?`)) return;
...
function initializeExtension() {
    ...
    console.log(`[${MODULE_NAME}] 로드 완료 (파일명 저장 방식 적용 - 영구 지속)`);
}
